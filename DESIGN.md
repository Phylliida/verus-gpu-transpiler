...
- **G1 — the probe, then the pass.** Probe: hand-generate the certificate obligation for `add_limbs_to` and measure discharge automation under the Lean backend — this is the §5.2 gate. Then: the fork pass (`--emit-gpu`) lowering SST → KIR literal + obligation, straight-line fns first, loops second. Exit: protolith `sum_sq3`/`torus_dist2`/`wrap_delta` certificates with zero hand proof; `add_limbs_to` with at most localized hand lemmas.
  **Status 2026-07-16: probe PASSED** — `probe-g1/` (37 verified, 0 errors, closer-free); both bisimulation lemmas first-try under a fully mechanical five-shape schema. See `probe-g1/REPORT.md` for the schema, discovered constraints (`tactus_auto` attr on proof fns; mandatory-but-free `u_*` unfold layer; vstd Seq axioms suffice), and honest scoping (SST-reflection link is the remaining, engineering-class unknown).
  **Status 2026-07-16 (same day), G1a: the generator exists** — `kirgen/` emits certificate modules from typed kernel ASTs; first generation run verified unedited (52/0 crate total) on three real kernel shapes incl. Select/SubW/MulW seams. Design amendment: for kernels with exact functional postconditions, certificates target the spec fn — the chain exec == spec == KIR closes **without SST reflection**, demoting reflection to an optimization; §4.1's subset gains the requirement "exact functional postcondition". Remaining: loop-template emission, the SST adapter, cross-crate wiring. REPORT.md addendum has details.
